  ...
  return (
    <div className="p-4 sm:p-8 md:p-12 lg:p-16">
      <div className="flex flex-col justify-center items-center">
        <h1 className="text-center font-bold text-xl sm:text-2xl md:text-3xl mb-4">Our Community</h1>
          ...
          <div className="h-[300px] sm:h-[320px] md:h-[340px] lg:h-[350px] p-2 sm:p-3 md:p-4 overflow-visible">
            ...
            >
              {members.map((member, index) => (
                <SwiperSlide key={member.id} className="flex justify-center items-center h-fit px-2 sm:px-3 p-4 sm:p-5">
                  <div
                  ...
                  >
                    ...
                    <h2 className="text-base sm:text-lg font-semibold mb-2">{member.name}</h2>
                    <p className="text-green-700 font-medium text-sm sm:text-base mb-2">{member.role}</p>
            ...
            <button
              className="custom-prev absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 sm:-translate-x-6 md:-translate-x-8 lg:-translate-x-6 xl:-translate-x-8 z-20 w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 lg:w-14 lg:h-14 bg-green-900 hover:bg-green-500 rounded-full flex items-center justify-center text-white transition-all duration-300 hover:scale-110 shadow-lg hover:shadow-xl"
            >
              <ArrowLeft size={20} />
            </button>
            <button
              className="custom-next absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 sm:translate-x-6 md:translate-x-8 lg:translate-x-6 xl:translate-x-8 z-20 w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 lg:w-14 lg:h-14 bg-green-900 hover:bg-green-500 rounded-full flex items-center justify-center text-white transition-all duration-300 hover:scale-110 shadow-lg hover:shadow-xl"
            >
              <ArrowRight size={20} />
            </button>
          </div>
          ...
          <div className="custom-pagination flex justify-center mt-2 space-x-2 sm:space-x-3">
            {members.map((_, index) => (
              <button
                key={index}
                style={{
                  backgroundColor: index === activeIndex ? '#22c55e' : '#86efac',
